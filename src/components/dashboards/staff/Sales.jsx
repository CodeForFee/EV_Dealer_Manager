...
import { mockOrders, mockCustomers, mockVehicles } from '../../../data/mockData'

const Sales = () => {
  const [orders, setOrders] = useState(mockOrders.filter(order => order.user_id === 4)) // Filter for current staff
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="text-primary">💰 Bán hàng</h2>
        <Button 
          variant="primary" 
          className="ev-button"
      ...
      <Alert variant="info" className="mb-4">
        ...
        <p className="mb-0">
          Hôm nay bạn có {orders.filter(o => o.status === 'pending').length} đơn hàng chờ xử lý.
        </p>
      </Alert>
        ...
        <Col md={3}>
          <Card className="ev-card">
            <Card.Body className="text-center">
              ...
              <h3 className="text-info">{(totalRevenue / 1000000).toFixed(0)}M</h3>
            ...
            <Card.Body>
              <div className="mb-3">
                <div className="d-flex justify-content-between mb-1">
                  <span>Doanh thu:</span>
                  <strong>{(totalRevenue / 1000000000).toFixed(1)}B / 2.0B VNĐ</strong>
                </div>
                <ProgressBar 
                  now={(totalRevenue / 2000000000) * 100} 
                  variant="primary"
                  className="mb-2"
                />
              </div>
              <div className="mb-3">
                <div className="d-flex justify-content-between mb-1">
                  <span>Đơn hàng:</span>
                  <strong>{totalOrders} / 20 đơn</strong>
                </div>
                <ProgressBar 
                  now={(totalOrders / 20) * 100} 
                  variant="success"
                  className="mb-2"
                />
              </div>
              <div className="text-center">
                <small className="text-muted">
                  Còn {Math.max(0, 20 - totalOrders)} đơn hàng để đạt mục tiêu
                </small>
              </div>
            </Card.Body>
          </Card>
