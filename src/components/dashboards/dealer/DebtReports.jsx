...
import { mockDebts, mockCustomers, mockOrders, mockVehicles } from '../../../data/mockData'

const DebtReports = () => {
  const [debts, setDebts] = useState(mockDebts.filter(d => d.dealer_id === 1))
  ...
  const filteredDebts = debts.filter(debt => {
    ...
    if (filterStatus === 'overdue') return isOverdue(debt.due_date)
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="text-primary">💰 Báo cáo công nợ</h2>
        <Button 
          variant="primary" 
          className="ev-button"
      ...
      {overdueDebts > 0 && (
        <Alert variant="danger" className="mb-4">
          ...
          <p className="mb-0">
            Có {overdueDebts} khách hàng có công nợ quá hạn cần xử lý ngay.
          </p>
        </Alert>
      )}
      ...
      <Card className="ev-card">
        <Card.Header>
          <h5 className="mb-0">📋 Danh sách công nợ</h5>
        </Card.Header>
        <Card.Body>
          <Table responsive hover>
            ...
            <tbody>
              {filteredDebts.map((debt) => (
                <tr key={debt.id}>
                  <td>
                    <strong>{getCustomerName(debt.customer_id)}</strong>
                  </td>
                  <td>
                    <div>
      ...
      <Row className="g-4 mt-4">
        <Col lg={6}>
          <Card className="ev-card">
            <Card.Header>
              <h5 className="mb-0">📊 Tiến độ thu hồi</h5>
            </Card.Header>
            <Card.Body>
              <div className="mb-3">
